...
const updateUMKM = async (id, ownerId, name, sector, description, photo_url) => {
    ...
    if(photo_url && existing[0].photo_url && existing[0].photo_url !== '/uploads/umkm/default.png'){
        const oldPhotoPAth = path.join(process.cwd(), 'public', existing[0].photo_url);
...
const deleteUMKM = async (id, ownerId, role) => {
    ...
    if(existing[0].photo_url && existing[0].photo_url !== '/uploads/umkm/default.png'){
        const oldPhotoPAth = path.join(process.cwd(), 'public', existing[0].photo_url);
        if(fs.existsSync(oldPhotoPAth)){
            fs.unlinkSync(oldPhotoPAth);
        }
